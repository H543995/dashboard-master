...
class Login_manger extends StatefulWidget {
	@override
	_Login_mangerState createState() => _Login_mangerState();
}

class _Login_mangerState extends State<Login_manger> {
	...
	void login(context, load) async {
		...
		if (txtmobile.text.isNotEmpty && txtpwd.text.isNotEmpty) {
			isloading = true;
			load.add_loading();
			bool res = await loginmanger(txtmobile.text, txtpwd.text, context);
			isloading = res;
			load.add_loading();
		}
	}
